// pages/checked/checked.js
var app = getApp()
Page({
  data:{
    userInfo:{}
  },
  onLoad:function(options){
    // 生命周期函数--监听页面加载

    this.setData({userInfo:app.appData.userInfo})
    
    var cxt_arc = wx.createCanvasContext('firstCanvas');
    cxt_arc.setLineWidth(1);
    cxt_arc.setStrokeStyle('#d2d2d2');
    cxt_arc.setLineCap('round')
    cxt_arc.setShadow(4,4,4,'#888888')
    cxt_arc.beginPath();
    cxt_arc.arc(150, 150, 100, 0, 2 * Math.PI, false);
    cxt_arc.stroke();

    cxt_arc.setLineWidth(3);
    cxt_arc.setStrokeStyle('red');
    cxt_arc.setLineCap('round')
    cxt_arc.beginPath();
    cxt_arc.arc(150, 150, 100,  -1/2*Math.PI, -1/2*Math.PI+this.data.userInfo.count/31*2*Math.PI, false);
    cxt_arc.stroke();
    cxt_arc.draw();
  },
  onReady:function(){
    // 生命周期函数--监听页面初次渲染完成
    
  },
  onShow:function(){
    // 生命周期函数--监听页面显示
    
  },
  onHide:function(){
    // 生命周期函数--监听页面隐藏
    
  },
  onUnload:function(){
    // 生命周期函数--监听页面卸载
    
  },
  onPullDownRefresh: function() {
    // 页面相关事件处理函数--监听用户下拉动作
  
  },
  onReachBottom: function() {
    // 页面上拉触底事件的处理函数
    
  },
  onShareAppMessage: function() {
    // 用户点击右上角分享
    return {
      title: 'title', // 分享标题
      desc: 'desc', // 分享描述
      path: 'path' // 分享路径
    }
  }
})